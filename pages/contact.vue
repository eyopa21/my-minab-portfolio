        ...
        <div class="md:ml-12">
          <form class="px-2 pt-6 pb-8 mb-4 bg-bg_color rounded">
            ...
            <div class="mb-6 text-left">
               <nuxt-link @click="send()" class=" p-3 px-6 pt-2 text-primary text-center tracking-wider bg-bg_color border-2 border-primary  hover:text-black hover:bg-primary"> <svg v-if="loading" role="status" class="inline w-6 h-6 mr-2 text-gray-200 animate-spin dark:text-white fill-primary" viewBox="0 0 100 101" fill="none" xmlns="http://www.w3.org/2000/svg">
...
const options = {
  ...
  data: `{"personalizations":[{"to":[{"email":"jobtennis21@gmail.com"}],"subject":" ${formData.value.name} : ${formData.value.subject}"}],"from":{"email":"${formData.value.email}"},"content":[{"type":"text/plain","value":"${formData.value.message}"}]}`
};
...
axios.request(options).then(function (response) {
  ...
  if(process.client){
    loading.value = false;
    alert.value = 'You have successfully send your feedback!!!'
  }
  ...
  if(process.client){
    loading.value = false;
    alert.value = 'something went wrong, try again!!!!'
  }
});
